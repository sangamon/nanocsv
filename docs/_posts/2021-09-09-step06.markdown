...
...and we get proper location reports for parse failure, exhaustion and trailing data...

```
ERROR: ColumnParseFailure(java.time.format.DateTimeParseException: 
  Text '2000-02-20x' could not be parsed, unparsed text found at index 10,ParserPos(1,2))
ERROR: RowExhaustionFailure(ParserPos(1,2))
ERROR: ColumnParseFailure(java.lang.IllegalStateException: trailing data,ParserPos(1,3))
```
